...
SELECT COUNT(DISTINCT unique_key) AS num_trips FROM `bigquery-public-data.chicago_taxi_trips.taxi_trips` WHERE
...
SELECT COUNT(DISTINCT unique_key) AS num_trips FROM `bigquery-public-data.chicago_taxi_trips.taxi_trips` WHERE trip_start_timestamp
>= '2015-12-23' AND trip_start_timestamp <= '2015-12-27'
...
SELECT COUNT(DISTINCT unique_key) AS num_trips FROM `bigquery-public-data.chicago_taxi_trips.taxi_trips` WHERE extract(hour from
trip_start_timestamp) in (9,10,11,12) and extract(year from trip_start_timestamp) != 2016

-- 4.
-- How many trips started within the 9th hour of the day (i.e. hour = 9) on October 31st from all the years strictly before 2017?
SELECT COUNT(DISTINCT unique_key) AS num_trips
FROM `bigquery-public-data.chicago_taxi_trips.taxi_trips`
WHERE extract(hour from trip_start_timestamp) = 9
and extract(day from trip_start_timestamp) = 31
and extract(month from trip_start_timestamp) = 10
